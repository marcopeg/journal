...
export const FETCH_JOURNAL_ENTRIES = gql`
  query fetchEntries($top: date!, $bottom: date!) {
    journal_logs(
      order_by: { created_at_day: desc }
      where: {
        created_at_day: { _lte: $top, _gte: $bottom }
        journal_question: { show_in_journal: { _eq: true } }
      }
    ) {
      text
      user_id
...
const useJournalHistory = (
  ...
  const loadMore = () => {
    ...
    const bottom = formatDate(
      decreaseDate(initialDate, showRecordsRef.current + options.pageSize)
    );
